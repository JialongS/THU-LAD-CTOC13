...
int ode45(std::function<int(double t, const double* x, double* dx, const double* para)> fun, double* x,
          const double* para,
          double t0, double tf, int n, int& NumPoint, double* work, const double* AbsTol, double RelTol = 1.0e-3,
          int NormControl = 0, double MaxStep = -1, double InitialStep = -1, FILE* fid = nullptr);
